...
namespace EdsanBooking.Utilities
{
    public static class IdGenerator
    {
        public static string GenerateNextId<TEntity>(DbSet<TEntity> dbSet, string prefix, Expression<Func<TEntity, string>> idSelector) where TEntity : class
        {
            ...
            {
                return prefix + "00001"; // Starting ID if there is no last ID or an unexpected format
            }
            ...
            // Generate the new ID
            return prefix + nextIdNumber.ToString("D5"); // Adjust the D5 to accommodate the expected ID format
        }
    }
}
